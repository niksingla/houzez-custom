<?php 
global $post, $top_area, $map_street_view,$is_custom_enabled;
...
if($is_custom_enabled):
    ...
    if($term_id):
        switch($term_id){
            case 171:                
                get_template_part( 'property-details/custom-office-building');
                break;
        }
    endif;
    ...
    if ($layout): foreach ($layout as $key=>$value) {
        switch($key) {
            ...
            case 'overview-v2':
                get_template_part('property-details/overview-v2');
                break;
